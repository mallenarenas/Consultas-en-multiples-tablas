-- Desafio 3
-- Mallen Arenas
CREATE DATABASE desafio3_mallen_arenas_527;
...
SELECT posts.title, posts.content FROM posts INNER JOIN comments ON posts.id=comments.post_id GROUP BY posts.title, posts.content ORDER BY COUNT(comments.post_id) DESC LIMIT 1;
...
SELECT posts.title AS title_post, posts.content AS content_post, comments.content AS content_comment, users.email FROM users INNER JOIN comments ON users.id=comments.user_id INNER JOIN posts ON posts.id=comments.post_id;

-- 9. Muestra el contenido del último comentario de cada usuario.

SELECT created_at, content, user_id FROM comments WHERE id IN (SELECT MAX(id) FROM comments GROUP BY user_id) ORDER BY user_id;

-- 10. Muestra los emails de los usuarios que no han escrito ningún comentario.
-- Hint: Recuerda el Having

SELECT email FROM users LEFT JOIN comments ON users.id=comments.user_id WHERE comments.content IS NULL;